//
//  OneDayChartViewController.swift
//  htmlParser
//
...
class OneDayChartViewController: UIViewController {
    var imgURL : String = ""
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        observer = NotificationCenter.default.addObserver(forName: UPDATE_STOCK_CHARTS, object: nil, queue: nil) {
            notification in
            print("oneDayChartNotification")
            self.downloadPic(page: (URL(string:(self.tabBarController as! StockChartTabBarController).imgURL[0])!))
        }
    }
